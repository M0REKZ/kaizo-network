...
void CEditorActionEditLayerTilesProp::Undo()
{
	...
	if(m_Prop == ETilesProp::WIDTH || m_Prop == ETilesProp::HEIGHT)
	{
		...
		if(pLayerTiles->m_HasGame || pLayerTiles->m_HasFront || pLayerTiles->m_HasSwitch || pLayerTiles->m_HasSpeedup || pLayerTiles->m_HasTune || pLayerTiles->m_HasKZGame || pLayerTiles->m_HasKZFront)
		{
			...
			if(Map()->m_pKZGameLayer && !pLayerTiles->m_HasKZGame)
				RestoreLayer(KZ_LAYERTYPE_GAME, Map()->m_pKZGameLayer);
			if(Map()->m_pKZFrontLayer && !pLayerTiles->m_HasKZFront)
...
void CEditorActionEditLayerTilesProp::Redo()
{
	...
	if(m_Prop == ETilesProp::WIDTH || m_Prop == ETilesProp::HEIGHT)
	{
		...
		if(pLayerTiles->m_HasGame || pLayerTiles->m_HasFront || pLayerTiles->m_HasSwitch || pLayerTiles->m_HasSpeedup || pLayerTiles->m_HasTune || pLayerTiles->m_HasKZGame || pLayerTiles->m_HasKZFront)
		{
			...
			if(Map()->m_pKZGameLayer && !pLayerTiles->m_HasKZGame)
				Map()->m_pKZGameLayer->Resize(pLayerTiles->m_Width, pLayerTiles->m_Height);
			if(Map()->m_pKZFrontLayer && !pLayerTiles->m_HasKZFront)
