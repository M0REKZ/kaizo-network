...
void CGameClient::OnInit()
{
	...
	const char *pLoadingDDNetCaption = Localize("Loading Kaizo Client"); //+KZ modified
...
CGameInfo GetGameInfo(const CNetObj_GameInfoEx *pInfoEx, int InfoExSize, const CServerInfo *pFallbackServerInfo)
{
	...
	if(Version < 1)
	{
		...
		Vanilla = str_comp(pGameType, "DM") == 0 || str_comp(pGameType, "TDM") == 0 || str_comp(pGameType, "CTF") == 0 || str_comp(pGameType, "LMS") == 0 || str_comp(pGameType, "LTS") == 0;
		...
		for(int i = 0; i < Num; i++)
		{
			...
			if(Item.m_Type == NETOBJTYPE_CLIENTINFO)
			{
				...
				if(ClientId < MAX_CLIENTS)
				{
					...
					if(!pClient->m_CustomClient && IsCustomClientId(pClient->m_Country)) //+KZ added
					{
						pClient->m_CustomClient = pClient->m_Country;
					}
	...
	{
		{
			...
			if(g_KaizoConfig_KaizoFakeMaxZoom)
			{
				Msg.m_X = std::numeric_limits<int>::max();
				Msg.m_Y = std::numeric_limits<int>::max();
			}
			else
			{
				float x, y;
...
void CGameClient::OnPredict()
{
	...
	bool RealPredTick = false; //TClient
	...
	m_PredictedTick = FinalTickRegular; //+KZ modified for fast input commit
...
void CGameClient::SendStartInfo7(bool Dummy)
{
	...
	{
		//+KZ modified
		Msg.m_apSkinPartNames[p] = IsSkinPartDefault(Dummy, p) ? "kaizo!" : CSkins7::ms_apSkinVariables[(int)Dummy][p];
...
void CGameClient::UpdateRenderedCharacters()
{
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if(Predict() && (i == m_Snap.m_LocalClientId || (AntiPingPlayer && !IsOtherTeam(i))) && pChar)
		{
			...
			// +KZ From TClient
			if(g_KaizoConfig_KaizoFastInput && (i == m_Snap.m_LocalClientId || IsDummy))
