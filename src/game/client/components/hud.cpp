...
extern int g_KaizoConfig_KaizoShowTrackedAmmo;
...
void CHud::RenderAmmoHealthAndArmor(const CNetObj_Character *pCharacter)
{
	...
	//+KZ
	if(GameClient()->m_GameInfo.m_HudAmmo && g_KaizoConfig_KaizoShowTrackedAmmo)
	{
		...
		for(int i = 0; i < NUM_WEAPONS; i++)
		{
			...
			if(TrackedAmmo[i] >= 0)
			{
				str_format(abuf, sizeof(abuf), "%d", TrackedAmmo[i]);
			}
			else
			{
	...
	else if(GameClient()->m_GameInfo.m_HudAmmo)
	{
		// ammo display
		float AmmoOffsetY = GameClient()->m_GameInfo.m_HudHealthArmor ? 24 : 0;
		int CurWeapon = pCharacter->m_Weapon % NUM_WEAPONS;
	...
	if(GameClient()->m_GameInfo.m_HudHealthArmor)
	{
		...
		if(SplitsWidth != 0.0f) /* CMClient */
			Graphics()->RenderQuadContainerEx(m_HudQuadContainerIndex, m_HealthOffset + QuadOffsetSixup, DisplayHealth, SplitsWidth /* CMClient */, 0);
		else
		...
		if(SplitsWidth != 0.0f) /* CMClient */
			Graphics()->RenderQuadContainerEx(m_HudQuadContainerIndex, m_EmptyHealthOffset + QuadOffsetSixup + DisplayHealth, 10 - DisplayHealth, SplitsWidth /* CMClient */, 0);
		else
		...
		if(SplitsWidth != 0.0f) /* CMClient */
			Graphics()->RenderQuadContainerEx(m_HudQuadContainerIndex, m_ArmorOffset + QuadOffsetSixup, DisplayArmor, SplitsWidth /* CMClient */, 0);
		else
		...
		if(SplitsWidth != 0.0f) /* CMClient */
			Graphics()->RenderQuadContainerEx(m_HudQuadContainerIndex, m_ArmorOffset + QuadOffsetSixup + DisplayArmor, 10 - DisplayArmor, SplitsWidth /* CMClient */, 0);
		else
