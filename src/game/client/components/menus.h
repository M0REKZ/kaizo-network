...
private:
	...
	bool m_OnlinePlayersCacheDirty = true; //From EClient
public:

	//CGenericPlayerInfo is from EClient
	...
	public:
		...
		CGenericPlayerInfo(const CServerInfo::CClient *pOnlineClient, const CServerInfo *pServer)
		{
			...
			{
				str_copy(m_aaSkin7[i], pOnlineClient->m_aaSkin7[i]);
				m_aUseCustomSkinColor7[i] = pOnlineClient->m_aUseCustomSkinColor7[i];
				m_aCustomSkinColor7[i] = pOnlineClient->m_aCustomSkinColor7[i];
			}
		}
	...
	std::vector<CGenericPlayerInfo> m_vKaizoUsersCache; //From EClient
