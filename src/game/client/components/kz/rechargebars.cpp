...
void CRechargeBars::RenderFreezeBar(const int ClientId)
{
	...
	float FreezeProgress = std::clamp(RechargeTick - (g_Config.m_ClPredict ? Client()->PredGameTick(g_Config.m_ClDummy) : GameClient()->m_GameWorld.GameTick()), 0, RechargeTime) / (float)RechargeTime;
...
void CRechargeBars::RenderFreezeBarPos(float x, const float y, const float Width, const float Height, float Progress, const float Alpha)
{
	...
	Graphics()->QuadsBegin();
	Graphics()->SetColor(0.3f, 0.3f, 0.3f, Alpha);
