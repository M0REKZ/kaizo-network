...
CKZGun::CKZGun(CGameWorld *pGameWorld, vec2 Pos, bool Damage, bool Explosive, int Layer, int Number) :
	CEntity(pGameWorld, CGameWorld::CUSTOM_ENTTYPE_KZGUN)
{
...
void CKZGun::Fire()
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if(aIsTarget[i])
		{
			...
			new CKZPlasma(&GameServer()->m_World, m_Pos, normalize(pTarget->m_Pos - m_Pos), m_Damage, m_Explosive, i);
			GameServer()->CreateSound(m_Pos,SOUND_HOOK_LOOP,pTarget->TeamMask());
		}
	}
}
...
void CKZGun::Snap(int SnappingClient)
{
